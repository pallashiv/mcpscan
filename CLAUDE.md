...
src/mcpscan/
  ...
  lock.py      Rug-pull detection (--lock/--write-lock): LOCK001-3
  cli.py       argparse entrypoint: `mcpscan scan <path>`
action.yml    GitHub Action (composite); scripts/run-scan.sh is its scan step
site/         Live demo (GitHub Pages, .github/workflows/pages.yml). index.html is hand-written;
               site/examples/*.html is generated by scripts/build_demo_site.py (gitignored) from
               the CURRENT code on every deploy, so it can't go stale relative to the rules.
